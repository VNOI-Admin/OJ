...
(function ($) {
    ...
    function computeVirtualRanking(virtualSubsData, rankingData, elapsed) {
        ...
        var newParts = rankingData.participations
            .filter(function (p) { return p.real_start == null; })
            .map(function (p) {
        ...
        var virtualPart = null;
        for (var i = 0; i < rankingData.participations.length; i++) {
            if (rankingData.participations[i].real_start != null) {
                virtualPart = rankingData.participations[i];
                break;
            }
        }
        if (virtualPart && virtualPart.subs) {
            var vProbMap = {};
            for (var j = 0; j < virtualPart.subs.length; j++) {
                var s = virtualPart.subs[j];
                if (s[3] > elapsed) continue;
                if (!vProbMap[s[0]]) vProbMap[s[0]] = [];
                vProbMap[s[0]].push({ pts: s[1], skip: s[2], t: s[3] });
            }
            var vScored = scoreOne(vProbMap);
            newParts.push({
                id: virtualPart.id, score: vScored.score, cumtime: vScored.cumtime,
                tiebreaker: vScored.tiebreaker, format_data: vScored.format_data,
                is_disqualified: virtualPart.is_disqualified, virtual: virtualPart.virtual,
                rating: virtualPart.rating, user: virtualPart.user,
            });
        }
    ...
    window.initVirtualRanking = function (rankingData, contestKey) {
        var virtualSubsUrl = rankingData.contest.virtual_subs_url;
        ...
        function tick() {
            ...
            var elapsed = (Date.now() / 1000) - virtualPart.real_start;
            if (elapsed >= virtualSubsData.duration) {
